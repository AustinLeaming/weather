...
class MakeApiCall:
    ...
    def submit_values(self, feels_like, temp, humidity):
        statsd.gauge('weather.outside.feels_like', feels_like, tags=["source:script.py"])
        statsd.gauge('weather.outside.temp_f', temp, tags=["source:script.py"])
        statsd.gauge('weather.outside.humidity', humidity, tags=["source:script.py"])
